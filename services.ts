...
export const markAttendance = async (userId: string, coordinates: { latitude: number, longitude: number } | null): Promise<AttendanceRecord> => {
    ...
    if (coordinates && college && college.latitude && college.longitude) {
        distanceInKm = getDistanceInKm(coordinates.latitude, coordinates.longitude, college.latitude, college.longitude);
        const radiusInKm = (college.radius || 500) / 1000;
...
export const changePassword = async (userId: string, oldPass: string, newPass: string): Promise<{ success: boolean; message: string }> => {
    ...
    if (!user) return delay({ success: false, message: "User not found" });
    if (user.password !== oldPass) return delay({ success: false, message: "Incorrect old password" });
    
    user.password = newPass;
    storage.setItem('MOCK_USERS', users);
    MOCK_USERS = users;
    return delay({ success: true, message: "Password changed successfully" });
};
